...
input_likelihood = {"RSlowDSlowRISKvhigh": 0.8, "RSlowDShighRISKhigh": 0.6, "RShighDSlowRISKlow": 0.4, "RShighDShighRISKvlow": 0.2}

#####evaluate the price preference of each data-provider
dp_p_pref_price_wo_LCD = price_min_LCD + (30*(price_max_LCD-price_min_LCD)/100)
dp_p_pref_price_w_LCD = price_min_LCD + (30*(price_max_LCD-price_min_LCD)/100)
dp_p_pref_price_wo_HCD = price_min_HCD + (30*(price_max_HCD-price_min_HCD)/100)
dp_p_pref_price_w_HCD = price_min_HCD + (30*(price_max_HCD-price_min_HCD)/100)

dp_f_pref_price_wo_LCD = price_min_LCD + (30*(price_max_LCD-price_min_LCD)/100)
dp_f_pref_price_w_LCD = price_min_LCD + (30*(price_max_LCD-price_min_LCD)/100)
dp_f_pref_price_wo_HCD = price_min_HCD + (30*(price_max_HCD-price_min_HCD)/100)
dp_f_pref_price_w_HCD = price_min_HCD + (30*(price_max_HCD-price_min_HCD)/100)
...
pref_dp_u = {'w': [1/3, 1/3, 1/3], 'alpha': [0.1, 0.7, 0.2], 'WoRate_LCD': [0.1, 0, 0, 0, dp_p_pref_price_w_LCD], 'WoRate_HCD': [0.1, 0, 0, 0, dp_p_pref_price_w_HCD], 'WRate_LCD': [0.2, 0, 0, 0, dp_p_pref_price_w_LCD], 'WRate_HCD': [0.2, 0, 0, 0, dp_p_pref_price_w_HCD]}

pref_dp_p = {'w': [1/3, 1/3, 1/3], 'alpha': [0.1, 0.7, 0.2], 'WoRate_LCD': [0, 0, 0, 0, dp_p_pref_price_w_LCD], 'WoRate_HCD': [0, 0, 0, 0, dp_p_pref_price_w_HCD], 'WRate_LCD': [0, 0, 0.5, 0.5, dp_p_pref_price_w_LCD], 'WRate_HCD': [0, 0, 0.5, 0.5, dp_p_pref_price_w_HCD]}

pref_dp_f = {'w': [1/3, 1/3, 1/3], 'alpha': [0.1, 0.7, 0.2], 'WoRate_LCD': [0, 0.5, 0, 0.5, dp_f_pref_price_w_LCD], 'WoRate_HCD': [0, 0.5, 0, 0.5, dp_f_pref_price_w_HCD], 'WRate_LCD': [0, 0.5, 0.5, 0, dp_f_pref_price_w_LCD], 'WRate_HCD': [0, 0.5, 0.5, 0, dp_f_pref_price_w_HCD]}
...
for iter in range(0,iterations):
	...
	for x in range(0,N_buyer):
		...
		delta = round((price_max_LCD - price_min_LCD+1)/2)
		
		tr_LCD = list(np.random.randint(1,3, size=6))
		
		if tr_LCD[3] == 1:
			tr_LCD.append(np.random.randint(price_min_LCD, price_min_LCD+delta))
		elif tr_LCD[3] == 2:
			tr_LCD.append(np.random.randint(price_min_LCD+delta, price_max_LCD))
	
		
		delta = round((price_max_HCD - price_min_HCD+1)/2)
		
		tr_HCD = list(np.random.randint(2,4, size=6))
		
		if tr_HCD[3] == 2:
			tr_HCD.append(np.random.randint(price_min_HCD, price_min_HCD+delta))
		elif tr_HCD[3] == 3:
			tr_HCD.append(np.random.randint(price_min_HCD+delta, price_max_HCD))
