...
#deploy rule contracts
rule_address, abiRule, rule_gasUsed = setup.deployRulecontracts(URL)

#deploy privacy rating contracts
PrivacyRating_address, abiPrivacyrating, PR_gasUsed = setup.deployPrivacyRatingcontracts(URL, rule_address)

#deploy marketplace contracts
registerAddress, registerABI, register_gasUsed = setup.deployRegistercontracts(URL, PrivacyRating_address)
...
#add regulator
_Admin.addRegulator(entityList['Regulator']['address'])

#register Auditor
_Auditor.registerAuditor()
...
_Buyer1.uploadPrivacyPolicy(entityList['Buyer1']['filelocation'])

#create profile for Buyer1 in marketplace
_Buyer2.createProfileinMarketplace()
_Buyer2.uploadPrivacyPolicy(entityList['Buyer2']['filelocation'])

#create profile for Buyer1 in marketplace
_Buyer3.createProfileinMarketplace()
_Buyer3.uploadPrivacyPolicy(entityList['Buyer3']['filelocation'])

print('\n********************************************************************')
print(f'Auditor and regulator submit their report')
print('********************************************************************')

_Auditor.AuditReport(entityList['Buyer1']['address'], 1, 1, 1, 1, 1, 1, 100)    # P, V, G, Co, DS, R, time

_Regulator.InvestigationReport(entityList['Buyer1']['address'], 100, 5, 5, 5, 5)   #_time, _vol, _pro, _sens, _agree
...
_Provider1.retrieveRating(_Buyer1, _Provider1)
