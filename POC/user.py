...
class user:
    ...
    def uploadPrivacyPolicy(self, fileLocation):
        
        RegisterSc = self.contracts['marketplace']['contract']
        web3 = self.contracts['marketplace']['web3']

        username = self.profile['UserName']
        uploadPolicy_tx = RegisterSc.functions.uploadPolicy(fileLocation).buildTransaction(
        {
            'from': self.profile['address'],
            'gasPrice': web3.eth.gas_price,
            'nonce': web3.eth.getTransactionCount(self.profile['address']),
        ...
        tx_create = web3.eth.account.signTransaction(uploadPolicy_tx, self.profile['private_key'])
        tx_hash = web3.eth.sendRawTransaction(tx_create.rawTransaction)
        tx_receipt = web3.eth.waitForTransactionReceipt(tx_hash)
        print(f'{username} privacy policy added: {tx_receipt.gasUsed}')
        logs = RegisterSc.events.logID().processReceipt(tx_receipt, errors=DISCARD)
        #self.profile['UserID'] = Web3.toHex(logs[0]['args'][''])
        print(f'{username} uploaded the privacy policy handle in Marketplace')
    ...
    def retrieveRating(self, _buyer, _provider): #address of buyer for whom rating is needed
        RegisterSc = self.contracts['marketplace']['contract']
        #username = self.profile['UserName']
        
        (currentNonTemporal, currentTemporal, futureNonTemporal, futureTemporal, pastImpacts, currentImpact, Likelihood, Practice) = RegisterSc.functions.retrieveProfiles(_buyer.profile['address'], _provider.profile['address']).call()
        print(f'-> Current possession Non_temp (sample, dim, div) : {currentNonTemporal}')
        print(f'-> Current possession temp (time, interval, period, age): {currentTemporal}')
        print(f'-> Future possession Non_temp (buyer, sample, dim, div): {futureNonTemporal}')
        print(f'-> Future possession temp  (time, interval, period, age) : {futureTemporal}')
        print(f'-> Past impacts (_vol, _prov, _agree, _sens) : {pastImpacts}')
        print(f'-> Current impact (_vol, _prov, _agree, _sens) : {currentImpact}')
        print(f'-> Likelihood : {Likelihood}')
        print(f'-> Practice (P, V, G, Co, DS, R): {Practice}')
